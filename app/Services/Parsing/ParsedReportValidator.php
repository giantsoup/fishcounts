    ...
    private function missingSourceReportData(RawPayloadData $payload, ParsedFishCountCollection $parsed): array
    {
        ...
        return $paragraphs
            ...
            ->map(function (string $paragraph, int $index) use ($payload, $parsed, $paragraphs, $evidenceStrategy): ParsedReportValidationData {
                preg_match('/(?:The\s+)?(?<label>[A-Z][A-Za-z0-9 \'&.-]{1,60}?)(?:\s+(?:also\s+)?(?:just\s+)?(?:caught|returned|came\s+back|is\s+returning|is\s+up\s+to|had|has|finished(?:\s+up)?|ended|called(?:\s+in)?|checked(?:\s+in)?|\||\d+(?:\.\d+|\/\d+)?\s*Day))\b/i', $paragraph, $matches);
